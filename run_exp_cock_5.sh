...
do
        ...
        echo "Reloaded DB"
        ssh xcnc35 "cd $project_dir && git pull && bash run_node.sh $xact_dir 1 $n_clients $n_nodes ${ips[0]}" &
        ssh xcnc36 "cd $project_dir && git pull && bash run_node.sh $xact_dir 2 $n_clients $n_nodes ${ips[1]}" &
        ssh xcnc37 "cd $project_dir && git pull && bash run_node.sh $xact_dir 3 $n_clients $n_nodes ${ips[2]}" &
        ssh xcnc38 "cd $project_dir && git pull && bash run_node.sh $xact_dir 4 $n_clients $n_nodes ${ips[3]}" &
        ssh xcnc39 "cd $project_dir && git pull && bash run_node.sh $xact_dir 5 $n_clients $n_nodes ${ips[4]}" &
        wait
